...
describe('inag-table', () => {
  ...
  beforeEach(() => {
    element = document.createElement('inag-table');
  ...
  it('should check all row checkboxes by clicking the "check-all" checkbox', (done) => {
    ...
    let rows = element.shadowRoot.querySelector('tbody').querySelectorAll('tr');
    const checkAll = document.createElement('inag-checkbox');
    ...
    setTimeout(() => {
      const allChecked = [...rows].every(row => row.querySelector('inag-checkbox').checked);
  ...
  it('should toggle the class "selected" of a row when its checkbox is clicked', done => {
    ...
    const checkBox = row.querySelector('inag-checkbox');
